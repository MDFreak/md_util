...
bool Button2::operator==(Button2 &rhs) { return (this==&rhs); }
void Button2::setDebounceTime(unsigned int ms)          { debounce_time_ms = ms; }
void Button2::setChangedHandler(CallbackFunction f)     { change_cb = f;  }
void Button2::setPressedHandler(CallbackFunction f)     { pressed_cb = f; }
void Button2::setReleasedHandler(CallbackFunction f)    { released_cb = f; }
void Button2::setClickHandler(CallbackFunction f)       { click_cb = f; }
void Button2::setTapHandler(CallbackFunction f)         { tap_cb = f; }
void Button2::setLongClickHandler(CallbackFunction f)   { long_cb = f; }
void Button2::setDoubleClickHandler(CallbackFunction f) { double_cb = f; }
void Button2::setTripleClickHandler(CallbackFunction f) { triple_cb = f; }
unsigned int Button2::wasPressedFor()                   { return down_time_ms; }
boolean Button2::isPressed()                            { return (state == LOW); }
unsigned int Button2::getNumberOfClicks()               { return click_count; }
unsigned int Button2::getClickType()                    { return last_click_type; }
void Button2::loop()
  {
    if(pin < 0)return;
    prev_state = state;
